...
public class Arbol_B {
    ...
    void insertar_nodo(int valor){ // recorrer el arbol para ver en que pagina tiene que insertar
        Nodo_B nuevo = new Nodo_B(valor);
        if(this.raiz == null){
    ...
    Object insertar_recorrer(Nodo_B nuevo, Pagina raiz_actual){ //Metodo recursivo para buscar la pagina en la que se debe de insertar
    ...
    void Graficar_ArbolB() throws IOException{
       String path = System.getProperty("user.home") + "\\Desktop\\ArbolB.dot"; // el metodo system.getProperty se utiliza para obtener la ruta
        ...
        bw = new BufferedWriter(new FileWriter(archivo));
            bw.write("digraph arbolB{\n"
                ...
                +recorer_GraficarNodos(this.raiz) //---- llamamos al metodo recursivo que graficara las paginas y las claves
                +recorer_GraficarEnlaces(this.raiz) //*----- metodo recursivo que graficara los enlaces
                +"}\n");
        bw.close();
         String do_path = "C:\\Program Files\\Graphviz\\bin\\dot.exe"; //----- esta ruta debe ser donde este instalado el dot.exe de graphviz 
        ...
        System.out.print("-----------"+file_input_path+"\n") ;
                
        try {
            ProcessBuilder pBuilder; //---- process builder nos ayudara a ejecutar comandos de consola para compilar el .dot
    ...
    String recorer_GraficarNodos(Pagina raiz_actual){
        ...
        if(raiz_actual.Es_hoja(raiz_actual)){//si es hoja solo se escribe su nodo 
            ...
            int cont=0; //-- contador de claves de pagina
            Nodo_B pivote = raiz_actual.claves.getPrimero();
            while(pivote != null){
                cont++;
